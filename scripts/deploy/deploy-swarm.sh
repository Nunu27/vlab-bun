...
if [[ -n "$WORKER_TOKEN" && -n "$MANAGER_IP" ]]; then
  echo
  warn "Workers do NOT run inside Docker Swarm (containerlab requires --privileged)."
  echo
  echo "  On each worker node, run:"
  echo
  if [[ -n "$PUBLIC_IP" && "$MANAGER_IP" != "$PUBLIC_IP" ]]; then
    echo "    # 1. Join swarm (choose the IP reachable from your worker node)"
    echo "    docker swarm join --token ${WORKER_TOKEN} --advertise-addr <WORKER_IP> ${PUBLIC_IP}:2377   (Public)"
    echo "    docker swarm join --token ${WORKER_TOKEN} --advertise-addr <WORKER_IP> ${MANAGER_IP}:2377  (Internal)"
  else
    echo "    # 1. Join swarm"
    echo "    docker swarm join --token ${WORKER_TOKEN} --advertise-addr <WORKER_IP> ${MANAGER_IP}:2377"
  fi
  echo
  echo "    # 2. Deploy the worker as a privileged systemd service"
  echo "    curl -fsSL https://raw.githubusercontent.com/nunu27/vlab-bun/main/scripts/deploy/worker-node.sh | bash"
fi
echo
