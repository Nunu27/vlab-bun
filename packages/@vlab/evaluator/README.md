...
- **Pluggable Handlers**: Supports custom evaluation contexts for different target nodes (e.g., Linux, MikroTik).
- **Reactive Engine**: Embraces a dependency-mapped design to evaluate checks immediately as system state changes.
- **Intelligent Resource Allocation**: Connects to and streams only from sources that are required by active checks.
- **Debounced Source Reads**: Batches system read operations efficiently under high-frequency stream events.
...
// 1. Get all registered handlers and checks
const registry = evaluator.getChecks();
console.log("Supported handlers & checks:", registry);

// 2. Spin up an evaluation session for a lab
const session = evaluator.createSession(
...
// 3. Listen to check status changes
session.emitter.on("change", ({ checkId, passed }) => {
  console.log(`Check ${checkId} status: ${passed ? "PASS" : "FAIL"}`);
});

// 4. Start the evaluation session
await session.start();
...
- **Source: `rip-interface-template`** (Listens via `/routing/rip/interface-template/listen`)
