...
import { RosApiMenu, RouterOSClient, RStream } from "routeros-client";
...
export default {
	...
	async checkAccess({ host, logger }, { id, ports }) {
		...
		try {
			...
			logger.warn("MikroTik ROS monitor: Access failed for node %s");
			return false;
		}
	},
		...
		try {
			...
			listener.on("data", (data) => {
				...
				if (dead) {
					...
					if (ifaceId in idMap) {
						const index = interfaces[data.interface]!.indexOf(
							idMap[ifaceId]!.address
						);

						if (index === -1) return;

						interfaces[data.interface]![index] = address;
						idMap[ifaceId]!.address = address;
					} else {
						idMap[ifaceId] = { iface: data.interface, address };
						interfaces[data.interface]!.push(address);
					}
				}
	...
	async extractInterfaces({ host, logger }, _, { id, ports }) {
		if (nodeInterfaceMap.has(id)) return nodeInterfaceMap.get(id)!;
		...
		try {
			...
			const data: any[] = await api.menu("/ip address").print();
			const idMap: Record<string, { iface: string; address: string }> = {};
			const interfaces: Record<string, string[]> = data.reduce(
				(acc, { id, interface: iface, address }) => {
					if (iface in acc) acc[iface]!.push(address);
